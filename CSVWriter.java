...
public class CSVWriter {
	...
	public static void writeConfCSV(String fileName, ArrayList<Turtle.Conflict> list) {
		String cString, turtle, dirC, lane, ped, dirP, ying, vel, dec, nDec, ttc, range, yDist, 
			time, init, sinceD, timeD, conn, auto, pedGap, brt, sinceBlocked;
		String comma  = ",";
		String nLine  = "\n";
		String header = "turtle,dirC,lane,ped,dirP,ying,sinceD,timeD,vel,dec,nMaxDec,ttc,range,yDist,time,init,connected,automated,pedGap,BRT,sinceBlocked" + nLine;
		...
		try {
			...
			for (Turtle.Conflict c : list) {
				...
				Double yDistM	= c.yDist*UserPanel.spaceScale;
				String yDist0	= form.format(yDistM);
				...
				cString = turtle + dirC + lane + ped + dirP + ying + sinceD + timeD + vel + dec + nDec + 
						ttc + range + yDist + time + init + conn + auto + pedGap + brt + sinceBlocked + nLine;
